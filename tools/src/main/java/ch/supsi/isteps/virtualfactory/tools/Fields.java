...
public class Fields implements Serializable{
	...
	private static final long serialVersionUID = -1694321997722916463L;
	...
	public static Map<String, String> toMap(Fields someFields) {
		HashMap<String, String> result = new HashMap<String, String>();
		for (String each : someFields.keys()) {
			result.put(each, someFields.firstValueFor(each));
		}
		return result;
	}

	public static Fields fromMap(Map<String, String> aMap) {
		Fields result = Fields.empty();
		for (String each : aMap.keySet()) {
			result.put(each, aMap.get(each));
		}
		return result;
	}

	public void copyOn(HashMap<String, String> someProperties) {
	}
}
